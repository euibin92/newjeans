...
$(document).ready(function(){
	...
	function showList(p){            		
		...
		$.getJSON("/replies/pages/"+bno+"/"+page+".json",function(data){
			...
			for(var i=0,len=data.list.length||0 ; i<len ; i++){
				...
				console.log(data.list[i].deleted+"asda");
				str+="<li style='cursor:pointer;' data-rno='"+data.list[i].rno+"'>";
				str+="	<div>";
				str+="		<div class='header'>";
				str+="            <strong class='primary-font'>" + (data.list[i].deleted == 'Y' ? data.list[i].replyer : data.list[i].replyer) + "</strong>";
	 			str+="			<small class='pull-right text-muted'>"+displayTime(data.list[i].replyDate)+"</small>";
				str+="		</div>";
				
				str+="		<p>"+(data.list[i].deleted=='Y'?'삭제된 댓글입니다': data.list[i].reply )+"</p>";
		
				
				if(replyer === data.list[i].replyer &&data.list[i].deleted!=='Y' || isAdmin&&data.list[i].deleted!=='Y'){
	...
	$(document).ready(function() {
		...
		$(".chat").on("click", "#delBtn", function(e) {
			...
			$("#RemoveModal").data("rno", rno).modal("show"); // 모달 창을 띄우기
		});
	});
